...
const DropdownUser = () => {
  const { currentUser } = useAuth(); // Get current user from the useAuth hook
  const [userProfile, setUserProfile] = useState<DocumentData | null>(null); // State for user profile
  const [userImage, setUserImage] = useState<string | null>(null); // State for user image

  useEffect(() => {
    if (currentUser) {
      fetchUserProfile(currentUser.uid); // Fetch profile data when user is logged in
    }
  }, [currentUser]);
  ...
  const fetchUserProfile = async (uid: string) => {
    ...
    try {
      const docRef = doc(db, "users", uid); // 'users' is the collection and uid is the document ID
      const docSnap = await getDoc(docRef); // Fetch the document
      if (docSnap.exists()) {
        setUserProfile(docSnap.data()); // Set user profile data from Firestore
      } else {
      }
    } catch (error) {
  ...
  while (userImage == null || userProfile == null) {
    return (
