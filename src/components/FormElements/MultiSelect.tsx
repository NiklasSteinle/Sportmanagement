...
const MultiSelect: React.FC<DropdownProps> = ({ id }) => {
  ...
  const select = (index: number, event: React.MouseEvent) => {
    ...
    setOptions(newOptions);
    updateSelectElement(newOptions);
  };
  ...
  const updateSelectElement = (newOptions: Option[]) => {
    const select = document.getElementById(id) as HTMLSelectElement | null;
    if (select) {
      for (let i = 0; i < select.options.length; i++) {
        select.options[i].selected = newOptions[i].selected;
      }
    }
  };
  ...
  return (
    <div className="relative z-50">
      ...
      <div>
        ...
        <select className="hidden" id={id} title="Multiselect Dropdown">
